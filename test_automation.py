...
DEFAULT_INPUT_COLUMN_CANDIDATES = ["Singlish","Input","Singlish Input","Test Input","Source","Sentence","Text"]
DEFAULT_EXPECTED_COLUMN_CANDIDATES = ["Sinhala","Expected_Output","Expected Output","Expected output","Expected","Expected Sinhala"]
DEFAULT_ACTUAL_COLUMN_CANDIDATES = ["Actual_Output","Actual Output","Actual output","Actual"]
DEFAULT_STATUS_COLUMN_CANDIDATES = ["Status","Result","Pass/Fail","Pass Fail"]
...
def _find_header_row(ws, max_scan_rows):
    ...
    for r in range(1, scan_limit + 1):
        ...
        for v in texts:
            ...
            if n in input_tokens: score += 3
            if n in expected_tokens: score += 2
            if n in actual_tokens: score += 1
            if n in status_tokens: score += 1
        if score > best_score:
            best_score = score
            best_row = r
    return best_row
...
def _find_column_index(header_values, requested_name, candidates):
...
def _last_header_col(header_values):
    last = 0
...
def _ensure_column(ws, header_row, header_values, desired_name):
...
def _dismiss_overlays(page):
    # Try common consent/close buttons
    candidates = [
        ("button", re.compile(r"^(Accept|I Agree|Agree|OK|Got it|Close|No thanks|Dismiss)$", re.IGNORECASE)),
...
def _safe_click(page, locator, timeout=15000):
    ...
    try:
        ...
        pass
    # Last resort: scroll into view then click
    try:
...
def _clear_textarea(page, locator, attempts=3):
    for _ in range(max(1, int(attempts))):
        # JS clear — fastest and most reliable
        try:
            locator.evaluate(
            ...
            pass
        # Safe click then keyboard clear
        _safe_click(page, locator)
        try:
...
def _ensure_input_value(page, input_locator, text, type_delay_ms):
    _clear_textarea(page, input_locator)
    page.wait_for_timeout(800)
    if type_delay_ms and int(type_delay_ms) > 0:
        _safe_click(page, input_locator)
...
def _read_output(is_chat, output_locator):
    if is_chat:
        try:
...
def run_test():
    ...
    with sync_playwright() as p:
        ...
        try:
            page.goto(args.url, wait_until="domcontentloaded", timeout=max(1000, int(args.timeout_ms)))
            try:
        ...
        for row_index in range(header_row + 1, int(ws.max_row or 0) + 1):
            ...
            try:
                ...
                if action_locator:
                    _safe_click(page, action_locator)
